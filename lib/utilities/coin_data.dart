...
class CoinData {
  Future getCoinData(String currency) async {
    ...
    for (String crypto in cryptoList) {
      String cryptoSign = crypto.split('/')[0];
      String requestExchange =
          '$kBaseURL/$cryptoSign/$currency?apikey=$kAPIKey';
      http.Response response = await http.get((Uri.parse(requestExchange)));
      if (response.statusCode == 200) {
        String cryptoName = crypto.split('/')[1];
        var data = jsonDecode(response.body);
        double price = data['rate'];
        CoinModel coins =
            CoinModel(icon: cryptoSign, name: cryptoName, price: price);
        cryptoPrices.add(coins);
      } else {
